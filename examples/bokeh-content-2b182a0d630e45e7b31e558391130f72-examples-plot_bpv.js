(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("77c08eb1-b08d-4ac8-9fd1-b477df071286");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '77c08eb1-b08d-4ac8-9fd1-b477df071286' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.0.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"70c81777-d6f9-437b-b6dd-26cbff5c7354":{"defs":[],"roots":{"references":[{"attributes":{"overlay":{"id":"17822"}},"id":"17816","type":"BoxZoomTool"},{"attributes":{},"id":"17815","type":"PanTool"},{"attributes":{},"id":"17814","type":"ResetTool"},{"attributes":{},"id":"17819","type":"UndoTool"},{"attributes":{},"id":"17860","type":"AllLabels"},{"attributes":{"data":{"x":[0,1],"y":[1,1]},"selected":{"id":"17864"},"selection_policy":{"id":"17863"}},"id":"17840","type":"ColumnDataSource"},{"attributes":{},"id":"17859","type":"BasicTickFormatter"},{"attributes":{"data":{},"selected":{"id":"17866"},"selection_policy":{"id":"17865"}},"id":"17846","type":"ColumnDataSource"},{"attributes":{"line_alpha":0.1,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17842","type":"Line"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17848","type":"Line"},{"attributes":{"coordinates":null,"formatter":{"id":"17859"},"group":null,"major_label_policy":{"id":"17860"},"ticker":{"id":"17807"}},"id":"17806","type":"LinearAxis"},{"attributes":{"coordinates":null,"data_source":{"id":"17840"},"glyph":{"id":"17841"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17843"},"nonselection_glyph":{"id":"17842"},"view":{"id":"17845"}},"id":"17844","type":"GlyphRenderer"},{"attributes":{"line_alpha":0.2,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17843","type":"Line"},{"attributes":{"children":[{"id":"17872"},{"id":"17870"}]},"id":"17873","type":"Column"},{"attributes":{"axis":{"id":"17810"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"17813","type":"Grid"},{"attributes":{},"id":"17817","type":"WheelZoomTool"},{"attributes":{"source":{"id":"17833"}},"id":"17838","type":"CDSView"},{"attributes":{},"id":"17804","type":"LinearScale"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17849","type":"Line"},{"attributes":{},"id":"17820","type":"SaveTool"},{"attributes":{},"id":"17798","type":"DataRange1d"},{"attributes":{"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17847","type":"Line"},{"attributes":{},"id":"17800","type":"DataRange1d"},{"attributes":{"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17841","type":"Line"},{"attributes":{"toolbar":{"id":"17871"},"toolbar_location":"above"},"id":"17872","type":"ToolbarBox"},{"attributes":{"bottom":1.459677659268402,"coordinates":null,"fill_alpha":0.1,"fill_color":"#2a2eec","group":null,"top":0.6850827603275131},"id":"17839","type":"BoxAnnotation"},{"attributes":{},"id":"17857","type":"AllLabels"},{"attributes":{"axis":{"id":"17806"},"coordinates":null,"group":null,"ticker":null},"id":"17809","type":"Grid"},{"attributes":{},"id":"17865","type":"UnionRenderers"},{"attributes":{"coordinates":null,"data_source":{"id":"17833"},"glyph":{"id":"17834"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17836"},"nonselection_glyph":{"id":"17835"},"view":{"id":"17838"}},"id":"17837","type":"GlyphRenderer"},{"attributes":{"source":{"id":"17840"}},"id":"17845","type":"CDSView"},{"attributes":{},"id":"17863","type":"UnionRenderers"},{"attributes":{},"id":"17856","type":"BasicTickFormatter"},{"attributes":{"coordinates":null,"formatter":{"id":"17856"},"group":null,"major_label_policy":{"id":"17857"},"ticker":{"id":"17811"}},"id":"17810","type":"LinearAxis"},{"attributes":{},"id":"17861","type":"UnionRenderers"},{"attributes":{"callback":null},"id":"17821","type":"HoverTool"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"17823","type":"PolyAnnotation"},{"attributes":{"tools":[{"id":"17814"},{"id":"17815"},{"id":"17816"},{"id":"17817"},{"id":"17818"},{"id":"17819"},{"id":"17820"},{"id":"17821"}]},"id":"17824","type":"Toolbar"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"17822","type":"BoxAnnotation"},{"attributes":{},"id":"17811","type":"BasicTicker"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"17852","type":"Title"},{"attributes":{"overlay":{"id":"17823"}},"id":"17818","type":"LassoSelectTool"},{"attributes":{},"id":"17802","type":"LinearScale"},{"attributes":{"coordinates":null,"data_source":{"id":"17846"},"glyph":{"id":"17847"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17849"},"nonselection_glyph":{"id":"17848"},"view":{"id":"17851"}},"id":"17850","type":"GlyphRenderer"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"17862"},"selection_policy":{"id":"17861"}},"id":"17833","type":"ColumnDataSource"},{"attributes":{},"id":"17862","type":"Selection"},{"attributes":{"source":{"id":"17846"}},"id":"17851","type":"CDSView"},{"attributes":{"below":[{"id":"17806"}],"center":[{"id":"17809"},{"id":"17813"},{"id":"17839"}],"height":500,"left":[{"id":"17810"}],"output_backend":"webgl","renderers":[{"id":"17837"},{"id":"17844"},{"id":"17850"}],"title":{"id":"17852"},"toolbar":{"id":"17824"},"toolbar_location":null,"width":500,"x_range":{"id":"17798"},"x_scale":{"id":"17802"},"y_range":{"id":"17800"},"y_scale":{"id":"17804"}},"id":"17797","subtype":"Figure","type":"Plot"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17835","type":"Line"},{"attributes":{"toolbars":[{"id":"17824"}],"tools":[{"id":"17814"},{"id":"17815"},{"id":"17816"},{"id":"17817"},{"id":"17818"},{"id":"17819"},{"id":"17820"},{"id":"17821"}]},"id":"17871","type":"ProxyToolbar"},{"attributes":{},"id":"17866","type":"Selection"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17834","type":"Line"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17836","type":"Line"},{"attributes":{},"id":"17864","type":"Selection"},{"attributes":{"children":[[{"id":"17797"},0,0]]},"id":"17870","type":"GridBox"},{"attributes":{},"id":"17807","type":"BasicTicker"}],"root_ids":["17873"]},"title":"Bokeh Application","version":"2.4.0"}}';
                  const render_items = [{"docid":"70c81777-d6f9-437b-b6dd-26cbff5c7354","root_ids":["17873"],"roots":{"17873":"77c08eb1-b08d-4ac8-9fd1-b477df071286"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();